 ...
 * */
const char *ServerErrors::what() const throw()
{
    ...
    switch (errorCode)
    {
    case ServerErrors::AddrInfoError:
        error = "GetAddrInfo Function Failed: ";
        break;
    case ServerErrors::SocketFdError:
        error = "GetSocketFd Function Failed: ";
        break;
    case ServerErrors::BindFdError:
            ...
            if (pfds[i].revents & POLLIN)
            {
                ...
                else
                {
                    ...
                    if (key <= 0)
                    {
                        ...
                        if (key < 0)
                            throw ServerErrors(ServerErrors::ReadError);
...
std::pair<struct sockaddr_storage, int> Server::accept_and_add_new_client() throw(ServerErrors)
{
    ...
    if (new_client_fd < 0)
        throw ServerErrors(ServerErrors::AcceptError);
